...
This fork includes an easy to build Docker image that can be ran on prerecorded video files (e.g. *.klg files or folders with RGB and Depth images). All you need is a recent version of Docker and the [Nvidia Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html).
...
It is highly recommended to precompute. For this you can use the utility script `./compute_masks.sh /path/to/input/dir /path/to/output/dir`. You can filter a subset of classes using the optional `--filter [CLASS 1] [CLASS  2] ...` argument. Copy the masks directly into the root of the input folder to directly use them with `run_on_folder.sh`. 
...
The optional flag `-ep` exports the poses per tracked object to files inside `./export`. Have a look at the scripts to modify this export location to your needs.
